...
class IrtTrainer:
    ...
    def _get_train_data(
        self, data: npt.NDArray, train_size: Union[int, float]
    ...
    def train(
        ...
        Y_train, _ = self._get_train_data(data, train_size)

        Ds = [5, 10]  # Dimensions to try
        device = "cpu"  # Either 'cuda' or 'cpu'
        epochs = (
            2000  # Number of epochs for IRT model training (py-irt default is 2000)
        )
        ...
        create_irt_dataset(Y_train, self.ave_dir / "irt_dataset.jsonlines")
        ...
        with open(self.save_dir / "lambds.pickle", "wb") as handle:
    ...
    def extract_anchors(
        ...
        for scenario, pos in scenarios_position.items():
            ...
            with open(
                self.save_dir / f"anchors/{scenario}_indices.pickle", "wb"
            ) as handle:
                pickle.dump(
            ...
            with open(
                self.save_dir / f"anchors/{scenario}_weights.pickle", "wb"
            ) as handle:
                pickle.dump(
